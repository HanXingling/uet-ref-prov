...
static void uet_desc_free(struct uet_ep *uet_ep)
{
	...
	if (uet_ep->rx_desc) {
		iov = (struct iovec *)uet_ep->rx_desc[0].buf_desc.iov.iov;
		if (iov)
			free(iov);
		free(uet_ep->rx_desc);
	}
	...
	if (uet_ep->tx_desc) {
		for (i = 0; i < uet_ep->num_tx_desc; i++) {
			...
			iov = (struct iovec *)uet_ep->tx_desc[i].buf_desc.iov.iov;
			if (iov)
				free(iov);
...
static ssize_t uet_send_req_api_common(
	...
	uint32_t job_id, const struct iovec *iov, size_t iov_count, uet_mr_handle_t mr_handle,
