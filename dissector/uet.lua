...
fld.flag_rreq_syn	= ProtoField.bool("uet.flags.rreq.syn",		"SYN",			16, yes_no, 0x04)
fld.flag_rreq_cc	= ProtoField.bool("uet.flags.rreq.cc",		"CC state present",	16, yes_no, 0x02)
...
fld.flag_ack_req		= ProtoField.uint16("uet.flags.ack.req",		"REQ",			base.DEC, nil, 0x06) -- TODO: enum
...
function p_uet.dissector(buf, pinfo, root)
	...
	if h_type == TYPE_RUD_REQ or h_type == TYPE_ROD_REQ then
		...
		flags_tree:add(fld.flag_rreq_rsv2, buf(offset, 2))
		...
		flags_tree:add(fld.flag_ack_p, buf(offset, 2))
